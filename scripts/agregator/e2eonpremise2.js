...
const driveUtils = require("../../modules/driveUtils_aggr");
...
Object.keys(datafile).forEach((dataKey) => {
  const reportsFolderPath = path.join(__dirname, `../../../Reports_${dataKey}`);
  ...
  const collectionPath = path.join(
    __dirname,
    // "../../collection_variables/api_collection/e2e_dynamic_update_or.json"
    "../../collection_variables/api_collection/e2e dynamic on premise scenario 2.json"
  );
  ...
  newman.run(newmanConfig, (err, summary) => {
    if (err) {
      ...
      driveUtils
        ...
        .then(() => console.log(`Drive upload completed for ${dataKey}.`))
        .catch((error) =>
          console.error(
            `Error during drive upload for ${dataKey}: ${error.message}`
          )
        );
    }
  });

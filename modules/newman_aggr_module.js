...
function generateCSVReport(reportPath, summary, dataKey) {
  const headers = [
    "Request Name",
    "API URL",
    "Status",
  ...
  summary.run.executions.forEach((execution) => {
      ...
      try {
        ...
        if (data && data[0]) {
          ...
          if (id && orNumber) {
            ...
            const logData = `Timestamp: ${timestamp}, DataKey: ${dataKey}, ORDER_ID: ${id}, OR_Number: ${orNumber}\n`;
    ...
    if (response?.stream) {
      try {
        ...
        //   parsedResponseBody
        // )}\n`;
        const logData = `Timestamp: ${timestamp}, DataKey: ${dataKey}, Response: ${JSON.stringify(
          parsedResponseBody
        )}\n`;
...
function getNewmanConfig(
  ...
  // Log all keys in the datafile
  console.log("Available data keys in the datafile:");
  Object.keys(fullData).forEach((key) => {
    console.log(key); // Log each key
  });
